...
export default async function CVPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const experienceItems = t.raw('experience.items') as Array<{ year: string; title: string; role: string; location: string; description: string; url?: string }>;
  const educationItems = t.raw('education.items') as Array<{ year: string; title: string; description: string; url?: string }>;
        ...
        <div className="grid grid-cols-12 gap-8 md:gap-12 print:gap-8 print:h-full">
                  ...
                  <div key={i} className="print-break-inside-avoid flex flex-col">
                    ...
                    <div className="flex flex-col gap-1 print:gap-0 text-base print:text-[14px] print:leading-tight">
                      {skillGroup.items.map((skill, j) => (
                        <span key={j} className="font-medium">{skill.name}</span>
                      ))}
                    </div>
                  </div>
                );
          ...
          <div className="col-span-12 md:col-span-8 print:col-span-8">
            ...
            <section className="mb-10 print:mb-8 text-base print:text-[14px] leading-relaxed relative flex items-start gap-4">
              <Scribble type="arrowRight" trigger="static" className="w-8 h-8 text-brand shrink-0 mt-1" />
              <p className="relative z-10">
                {t.rich('about.cvProfile', {
                  highlight: (chunks) => (
                    <span className="relative inline-block z-10 text-black font-medium">
                      {chunks}
                      <Scribble type="underline" trigger="static" className="absolute -bottom-1 left-0 w-full h-2 text-brand -z-10" />
                    </span>
                  )
                })}
              </p>
              ...
              {experienceItems.map((exp, i) => (
                <div key={i} className="lowercase print-break-inside-avoid flex flex-col">
                  ...
                  <div className="flex justify-between items-baseline mb-1 print:mb-0">
                    <h3 className="text-xl print:text-lg font-medium">
                      {exp.title}
                    </h3>
                  ...
                  <p className="text-base print:text-[14px] leading-relaxed">
                    {exp.description.split(/(<highlight>.*?<\/highlight>)/g).map((part, partIdx) => {
                      if (part.startsWith('<highlight>') && part.endsWith('</highlight>')) {
                        const inner = part.replace('<highlight>', '').replace('</highlight>', '');
                        return (
                          <span key={partIdx} className="relative inline-block z-10 text-brand font-medium">
                            {inner}
                          </span>
                        );
                      }
